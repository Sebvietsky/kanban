<script>
  ...
  import { deleteCard, updateCard, checkSpelling } from "../services/card.service";
  ...
  const updateSpelling = async(content) => {
    const data = await checkSpelling({prompt: content});
    console.log('data', data)
    const response = await updateCard({id: parseInt(card.id), content: data})
    card.content = response.content
  }
...
>
  ...
  >
    ...
    <button
      onclick={() =>  updateSpelling(card.content)}
      class="bg-yellow-500 text-white h-[50px] px-2 py-1 rounded"
    >
    Correction
    </button>
